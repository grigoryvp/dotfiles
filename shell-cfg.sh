...
WOX_CFG_WOX_KEYS="EnableAnonymousUsageStats"
WOX_CFG_DB="${HOME}/.wox/wox-user/wox.db"
WOX_CFG_SQL="${HHOME}/dotfiles/wox-settings.sql"
wox-cfg-export() {
  {
    echo "-- Generated by wox-cfg-export; apply with wox-cfg-import."
    # $(...) is word-split in zsh too, a bare $VAR is not
    for key in $(echo "$WOX_CFG_WOX_KEYS"); do
      sqlite3 "$WOX_CFG_DB" ".mode insert wox_settings" \
        "select key, value from wox_settings where key='${key}';"
    done
    for pair in $(echo "$WOX_CFG_KEYS"); do
      sqlite3 "$WOX_CFG_DB" ".mode insert plugin_settings" \
        "select plugin_id, key, value from plugin_settings
         where plugin_id='${pair%%:*}' and key='${pair#*:}';"
    done
  } | sed 's/^INSERT INTO/INSERT OR REPLACE INTO/' > "$WOX_CFG_SQL"
}
wox-cfg-import() {
  sqlite3 "$WOX_CFG_DB" < "$WOX_CFG_SQL" && echo "Restart Wox to apply."
}
