...
;;  m2-2 for game text
addRemap(["2", "appname", "Path of Exile 2"], ["m2"], ["send", "`"{!}(resistances|cursed|ailment application|break armour|less recovery|chilled ground|cooldown)`""])
addRemap(["2", "appname", "Path of Exile"], ["m2"], ["send", "`"{!}(non-curse|all maximum|cannot regenerate|less recovery|reflect)|((80|100)% deli)|memories`""])

;;  m2-3 for game text
;; FIXME: not working with 'shift'
addRemap(["3", "appname", "Path of Exile 2"], ["m2"], ["send", "`"{!}(resistances|cursed|ailment application|break armour|less recovery|chilled ground|cooldown)`""])
addRemap(["3", "appname", "Path of Exile"], ["m2"], ["send", "`"{!}(non-curse|all maximum|cannot regenerate|less recovery|reduced flask|taunted|reflect|goatmen|extra physical|projectiles)|((40|60|80|100)% deli)|memories`""])

;;  m2-4 for game text
addRemap("4", ["m2"], ["send", "`"greater black|grand black|exceptional black|coin|of the sun|scythe merc|vorana|olroth`""])

;;  m2-5 for game text
addRemap("5", ["m2"], ["send", "`"{!}(ap ti|client:|(nt|ed|r's) sh|incu|scar|ch ri|em so)`" `"(on|ck|ent|ver|en) oi|r's (p|b)|chay|ror sh|(ine|aos) o|of (kal|conta|awa|pre|cat|inv|bloodl|cal|corn|reg)|(ile|tic) ca|en su|deck`""])
