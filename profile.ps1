...
function mcd() { & mkdir $Args[0]; Set-Location $Args[0] }
...
# wox-settings.sql, see the same block in shell-cfg.sh. WOX_CFG_KEYS holds
# "plugin_id:key" pairs from the plugin_settings table, WOX_CFG_WOX_KEYS holds
# keys of the wox_settings table.
$WOX_CFG_KEYS = @(
  "979d6363-025a-4f51-88d3-0b04e9dc56bf:Disabled",
  "5f815d98-27f5-488d-a756-c317ea39935b:Disabled",
  "a48dc5f0-dab9-4112-b883-b68129d6782b:Disabled",
  "95d041d3-be7e-4b20-8517-88dda2db280b:Disabled",
  "1af58721-6c97-4901-b291-620daf08d9c9:Disabled",
  "8f68a760-86a0-46a9-b331-58dcaf091daa:Disabled",
  "78fc701b-a87e-4d5f-a7f2-13cbad9f7d1d:Disabled",
  "b8f3d4e5-6c7a-4b9c-8d1e-2f3a4b5c6d7e:Disabled",
  "6cde8bec-3f19-44f6-8a8b-d3ba3712d04e:Disabled",
  "a3f7b8c2-d1e4-4f6a-9b0c-7e2d1a5f8b3e:Disabled"
);
$WOX_CFG_WOX_KEYS = @(
  "OnboardingFinished", "EnableAnonymousUsageStats", "EnableAutostart@windows");
$WOX_CFG_DB = _path(@("~", ".wox", "wox-user", "wox.db"));
$WOX_CFG_SQL = _path(@("~", "dotfiles", "wox-settings.sql"));

function Export-WoxCfg() {
  $lines = @("-- Generated by Export-WoxCfg; apply with Import-WoxCfg.");
  foreach ($key in $WOX_CFG_WOX_KEYS) {
    $lines += & sqlite3 $WOX_CFG_DB ".mode insert wox_settings" `
      "select key, value from wox_settings where key='$key';";
  }
  foreach ($pair in $WOX_CFG_KEYS) {
    $pluginId, $key = $pair -split ":", 2;
    $lines += & sqlite3 $WOX_CFG_DB ".mode insert plugin_settings" `
      "select plugin_id, key, value from plugin_settings where plugin_id='$pluginId' and key='$key';";
  }
  $lines -replace "^INSERT INTO", "INSERT OR REPLACE INTO" | Set-Content -Path $WOX_CFG_SQL;
}

function Import-WoxCfg() {
  Get-Content -Path $WOX_CFG_SQL -Raw | & sqlite3 $WOX_CFG_DB;
  if ($LASTEXITCODE -eq 0) { Write-Host "Restart Wox to apply."; }
}
